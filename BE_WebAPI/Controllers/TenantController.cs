...
namespace BE_WebAPI.Controllers
{
    ...
    public class TenantController : ApiController
    {
        ...
        public IHttpActionResult GetEmployee(string id)
        {
            using (UserOperations _userOperation = new UserOperations())
            {
                Employee _user = _userOperation.GetEmployeeDetails(Convert.ToInt32(id));
                if (_user == null)
                    return NotFound();
                else
                    return Ok(_user);
        ...
        [HttpPost]
        [Route("PostComplaint")]
        public IHttpActionResult PostComplaint([FromBody]Complaint _complaint)
        {
            using (UserOperations _userOperation = new UserOperations())
            {
